export default {
  ...
  getLeftArr(date) {
    const arr = [];
    const leftNum = this.getMonthweek(date); // 这个月1号之前的上个月有几天）
    const num = this.getDaysInOneMonth(this.getOtherMonth(date, 'preMonth')) - leftNum + 1; // 上个月的总天数减去上月剩余的天数+1 等于当月显示的上个月剩余天数
